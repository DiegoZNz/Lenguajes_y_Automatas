...
if __name__ == '__main__':
    num_edges = int(input("Ingrese la cantidad de bordes: "))
    edges = []
    for _ in range(num_edges):
        source, dest, weight = map(int, input("Ingrese un borde (source dest weight): ").split())
        edges.append((source, dest, weight))
    
    N = max(max(source, dest) for source, dest, _ in edges) + 1  # Calcular N en función de los bordes ingresados
    graph = Graph(edges, N)
    source = 7
