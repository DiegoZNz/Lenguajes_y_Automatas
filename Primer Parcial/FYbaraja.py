...
def mostrar_cartas_jugador(jugador, num_jugador):
    messagebox.showinfo(f"Jugador {num_jugador + 1}", f"Jugador {num_jugador + 1}, tu baraja de siete cartas ha sido generada. ¡Revelar!")
    for i, carta in enumerate(jugador):
        messagebox.showinfo(f"Jugador {num_jugador + 1}", f"Tu carta número {i + 1} es: {carta}")
...
def main():
    ...
    for i, jugador in enumerate(jugadores):
        mostrar_cartas_jugador(jugador, i)
